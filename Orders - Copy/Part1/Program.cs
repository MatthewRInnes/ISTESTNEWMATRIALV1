...
// Brings my application namespace into scope; same as `Part1.csproj` RootNamespace (`Orders`).
using Orders;
...
// Prints my banner ID / candidate number on the second line of output.
Console.WriteLine("B01841977");

// SQLite DB file name; same as `Part1.csproj`:
// `<None Update="Orders.db">`
const string databaseFileName = "Orders.db";
...
// DbContext type name is `OrdersDbContext` (see `LearningPlatformsDbContext.cs`).
...
// Query: find one row whose CustomerName equals the typed text, ignoring upper/lower case differences.
// COMMENT PROBLEM: this comment mentions `DbSet<Car>` but your DbContext file uses `DbSet<Order>`.
// The name you actually query is `db.Orders` (DbSet property name), which represents SQL table `Orders` (Orders.sql).
var match = await db.Orders
    ...
    // - SQL column name: Orders.sql says `CustomerName TEXT NOT NULL`
    // - Entity mapping: LearningSite.cs says `[Column("CustomerName")] public string CustomerName { get; set; }`
    // - Query usage: this line must use `c.CustomerName`
...
if (match is null)
{
    // Explain that no row had thatCustomerName (including empty search).
    Console.WriteLine($"No Orders URL found for '{CustomerName}'.");
}
else
{
    // Prints the URL stored on the matching row.
    Console.WriteLine($"The Orders URL for {match.CustomerName} is {match.OrdersUrl}");
}
