...
const string databaseFileName = "TechForums.db";
...
var options = new DbContextOptionsBuilder<TechForumsDbContext>()
...
using var db = new TechForumsDbContext(options);
...
var match = await db.TechForums
    // Do not track entities in the change tracker (read-only lookup; slightly lighter).
    .AsNoTracking()
    // Keep rows where the stored Name matches the input after converting both to lower case.
