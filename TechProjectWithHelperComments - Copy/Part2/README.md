...
@model StudentServices.Pages.IndexModel @* ❌ same problem *@
```
...
@model StudentServices.Pages.IndexModel
```

### Quick “where is what?” map

- `Program.cs`: boots the website and wires up EF Core + Razor Pages
- `StudentServicesDbContext.cs`: the EF Core database gateway (the “Channels table in code”)
