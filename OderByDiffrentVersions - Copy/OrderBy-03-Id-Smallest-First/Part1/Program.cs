...
// *** Practise line for this folder only: lowest ID first (insert order / primary key order). ***
var rows = await db.MusicPlatforms
    .AsNoTracking()
    .OrderBy(x => x.ID)
    .ToListAsync();

Console.WriteLine("Ordered by ID ascending (smallest first):\n");
foreach (var r in rows)
    Console.WriteLine($"{r.ID,3} | {r.Name,-18} | {r.Url}");
