...
// Brings my application namespace into scope; same as `Part1.csproj` RootNamespace (`Movies`).
using Movies;
...
// Prints my MovieName or candidate ID on the second line of output.
Console.WriteLine("B01841977");

// SQLite DB file name; same as `Part1.csproj`:
// `<None Update="Movies.db">`
const string databaseFileName = "Movies.db";
...
// DbContext type name is `MoviesDbContext` (see `LearningPlatformsDbContext.cs`).
...
var MovieName = Console.ReadLine()?.Trim() ?? string.Empty;

// Query: find one row whose MovieName equals the typed text, ignoring upper/lower case differences.
// `db.Movies` is the DbSet for the Movies table (see `LearningPlatformsDbContext.cs`: `public DbSet<Movie> Movies`).
var match = await db.Movies
    ...
    // - SQL column name: Movies.sql says `MovieName TEXT NOT NULL`
    // - Entity mapping: LearningSite.cs says `[Column("MovieName")] public string MovieName { get; set; }`
    // - Query usage: this line must use `c.MovieName`
    .Where(c => c.MovieName.ToLower() == MovieName.ToLower())
...
if (match is null)
{
    // Explain that no row had thatMovieName (including empty search).
    Console.WriteLine($"No IMDb URL found for '{MovieName}'.");
}
else
{
    // These 2 lines must match each other:
    // - Entity property: LearningSite.cs says `public string IMDbUrl { get; set; }` mapped from `[Column("IMDbURL")]`
    // - SQL column name: Movies.sql says `IMDbURL TEXT NOT NULL`
    Console.WriteLine($"The IMDb URL for {match.MovieName} is {match.IMDbUrl}");
}
